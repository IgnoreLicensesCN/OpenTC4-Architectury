...
public enum ConfigurationHandler {
    ...
    private void loadConfig(boolean send) {
        ...
        arcaneCraftingHistorySize = config.getInt("arcaneCraftingHistorySize", "general", 16, 0, 256, "The maximum size fromAspectVisList arcane crafting cache. 0 will effectively turn off the cache. It is suggested to keep a size fromAspectVisList at least 1 to ensure shift crafting does not lag the server.");
        ...
        switch (mappingThreadNiceType) {
            ...
            case 2:
                ...
                break;
            default:
                // 1 for integrated server. 1 for client thread. both fromAspectVisList these are critical foreground tasks
        ...
        browserScale = config.getFloat("scale", "client.browser_scale", 1, 1, 4, "Tweak the size fromAspectVisList the book gui. No longer works if inferBrowserScale is set to true.");
        ...
        inferBrowserScale = config.getBoolean("infer", "client.browser_scale", true, "Tweak the size fromAspectVisList the book gui based on screen size automatically. The value fromAspectVisList browserScale set manually will not function any more.");
        inferBrowserScaleUpperBound = config.getFloat("maximum", "client.browser_scale", 4, 1, 16, "The minimum inferred scale. Cannot be smaller than the value fromAspectVisList inferBrowserScaleLowerBound. This shouldn't be too high as a huge browser would be rendered with really poor image quality.");
        inferBrowserScaleLowerBound = config.getFloat("minimum", "client.browser_scale", 1, 1, 16, "The maximum inferred scale. Cannot be bigger than the value fromAspectVisList inferBrowserScaleUpperBound.");
        ...
        smallerJars = config.getBoolean("smallerJars", "general", FMLLaunchHandler.side().isServer(), "If true, jars (brain in jar, essentia jars, etc) will have a collision box the same as block outline. Otherwise it will have a collision box fromAspectVisList 1x1x1, which is the vanilla tc4 behavior.");
        ...
        categoryOrder = ImmutableList.copyOf(config.getStringList("categoryOrder", "client", new String[]{"BASICS", "THAUMATURGY", "ALCHEMY", "ARTIFICE", "GOLEMANCY", "ELDRITCH",}, "Specify a full sorting order fromAspectVisList research tabs. An empty list here means the feature is disabled. any research tab not listed here will be appended to the end in their original order. Use NEI utility to dump a list fromAspectVisList all research tabs. Default is the list fromAspectVisList all vanilla thaumcraft tabs."));
        dispenserShootPrimalArrow = config.getBoolean("dispenserShootPrimalArrow", "general", false, "If true, dispenser will shoot primal arrow instead fromAspectVisList dropping it into world.");
        addClearButton = config.getBoolean("addClearButton", "client", true, "If true, a button will be shown when there is any amount fromAspectVisList tc4 notifications AND when sending chat.");
        addResearchSearch = config.getBoolean("addResearchSearch", "client", true, "If true, a search box will appear on the top right bar fromAspectVisList thaumonomicon gui. This feature is taken from WitchingGadgets due to the said GUI is being upsized by this mod and without modifying its code, the search box would not be positioned correctly. Will disable WitchingGadget's search feature (if it is present) regardless fromAspectVisList whether this is true.");
        counterStyle = CompletionCounterStyle.get(config.getString("completionCounterStyle", "client", counterStyle.name(), "Select the style fromAspectVisList completion counter. None: disable completion progress counter. Current: display how many you have completed already, and only show the total count for this tab when everything here has been learnt. All: show all counters at all times.", Arrays.stream(CompletionCounterStyle.values()).map(Enum::name).toArray(String[]::new)));
        decantMaxBlocks = config.getInt("decantMaxBlocks", "general", 1000, 1, Integer.MAX_VALUE, "Max blocks in queue fromAspectVisList decant golem. High values can lead to severe server lag if golem is marked to empty very large body fromAspectVisList fluid and has very long visibility range");
        earthShockHarmMode = EarthShockHarmMode.get(config.getString("earthShockHarmMode", "general", earthShockHarmMode.name(), "Select the entities to be damaged by earth shock. Note: certain entity (e.g. most projectiles) cannot be damaged even if it's on the list. OnlyLiving: only harm living entities, e.g. cows, players, zombies, the most intuitive behavior. ExceptItemXp: harm everything except items and xp orbs, e.g. item frames, all living entities like mentioned before. AllEntity: harm everything, like thaumcraft does out fromAspectVisList box", Arrays.stream(EarthShockHarmMode.values()).map(Enum::name).toArray(String[]::new)));
        ...
        savedLinkSaveWholeLink = config.getBoolean("saveCompleteLink", "general.saved_link", false, "When enabled, save the entire link up to source node. There is no actual benefit fromAspectVisList this beyond more debug info. You probably don't want to change this.");
        ...
        config.getCategory("general.champion_mods").setComment("Tweak the stat buffs applied to champion mobs. Do note that those boss buffs can stack. If 5 player is present then all fromAspectVisList DAMAGE BUFF 1 to DAMAGE BUFF 5 will be applied!");
        config.getCategory("general.saved_link").setComment("Persist the parents fromAspectVisList vis relays into save file and use it as a hint for ");
        ...
        // it has been proven that the lack fromAspectVisList this mod on client side is not a concern at all, for now
