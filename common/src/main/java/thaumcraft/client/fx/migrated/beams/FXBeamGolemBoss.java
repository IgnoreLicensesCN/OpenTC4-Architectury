...
public class FXBeamGolemBoss extends ThaumcraftParticle {
   ...
   public FXBeamGolemBoss(ClientLevel par1World, LivingEntity boss, Entity entity, float red, float green, float blue, int age) {
      super(par1World, boss.getX(), boss.getY(), boss.getZ(), 0.0F, 0.0F, 0.0F);
      this.boss = boss;
      float f1 = MathHelper.cos(-boss.getYRot() * ((float) Math.PI / 180F) - (float) Math.PI);
      float f2 = MathHelper.sin(-boss.getYRot() * ((float) Math.PI / 180F) - (float) Math.PI);
      float f3 = -MathHelper.cos(-boss.getXRot() * ((float) Math.PI / 180F));
      float f4 = MathHelper.sin(-boss.getXRot() * ((float) Math.PI / 180F));
      Vec3 v = new Vec3(f2 * f3, f4, f1 * f3);
      this.xo = this.x = boss.getX() + v.x * (double) 0.5F;
      this.yo = this.y = boss.getY() + (double) boss.getEyeHeight();
      this.zo = this.z = boss.getZ() + v.z * (double) 0.5F;
      this.rCol = red;
      this.gCol = green;
      this.bCol = blue;
   ...
   private void syncTCoords(){
      if (this.targetEntity != null) {
         var boundingBox = this.targetEntity.getBoundingBox();
         this.tX = this.targetEntity.xo;
         this.tY = boundingBox.minY + ((boundingBox.maxY - boundingBox.minY) / 2.0F);
         this.tZ = this.targetEntity.zo;
      }
   }
   ...
   public void tick() {
      ...
      this.ptY = this.tY;
      this.ptZ = this.tZ;
      float f1 = MathHelper.cos(-this.boss.getYRot() * ((float) Math.PI / 180F) - (float) Math.PI);
      float f2 = MathHelper.sin(-this.boss.getYRot() * ((float) Math.PI / 180F) - (float) Math.PI);
      float f3 = -MathHelper.cos(-this.boss.getXRot() * ((float) Math.PI / 180F));
      float f4 = MathHelper.sin(-this.boss.getXRot() * ((float) Math.PI / 180F));
      Vec3 v = new Vec3(f2 * f3, f4, f1 * f3);
      this.x = this.boss.getX() + v.x * (double) 0.5F;
      this.y = this.boss.getY() + (double) this.boss.getEyeHeight();
      this.z = this.boss.getZ() + v.z * (double) 0.5F;
      this.prevYaw = this.rotYaw;
      this.prevPitch = this.rotPitch;
      syncTCoords();
      float xd = (float)(this.x - this.tX);
...
//      GL11.glDisable(2884);
      ...
      RenderSystem.blendFunc(770, blendmode);
