...
public class BlockTaintFibreRenderer extends BlockRenderer implements ISimpleBlockRenderingHandler {
   ...
   public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
      ...
      if (metadata <= 4) {
         if (world.isSideSolid(x - 1, y, z, Direction.EAST, true) && world.getBlock(x - 1, y, z) != ConfigBlocks.blockTaint) {
            fix = false;
            renderer.renderFaceXPos(block, (float)x - 0.995F, y, z, block.getIcon(0, 0));
         }

         if (world.isSideSolid(x + 1, y, z, Direction.WEST, true) && world.getBlock(x + 1, y, z) != ConfigBlocks.blockTaint) {
            fix = false;
            renderer.renderFaceXNeg(block, (float)x + 0.995F, y, z, block.getIcon(0, 0));
         }

         if (world.isSideSolid(x, y, z - 1, Direction.SOUTH, true) && world.getBlock(x, y, z - 1) != ConfigBlocks.blockTaint) {
            fix = false;
            renderer.renderFaceZPos(block, x, y, (float)z - 0.995F, block.getIcon(0, 0));
         }

         if (world.isSideSolid(x, y, z + 1, Direction.NORTH, true) && world.getBlock(x, y, z + 1) != ConfigBlocks.blockTaint) {
            fix = false;
            renderer.renderFaceZNeg(block, x, y, (float)z + 0.995F, block.getIcon(0, 0));
         }

         if (world.isSideSolid(x, y - 1, z, Direction.UP, true) && world.getBlock(x, y - 1, z) != ConfigBlocks.blockTaint) {
            fix = false;
            renderer.renderFaceYPos(block, x, (float)y - 0.995F, z, block.getIcon(0, 0));
         }

         if (world.isSideSolid(x, y + 1, z, Direction.DOWN, true) && world.getBlock(x, y + 1, z) != ConfigBlocks.blockTaint) {
            fix = false;
      ...
      if ((metadata == 1 || metadata == 2) && world.isSideSolid(x, y - 1, z, Direction.UP, true)) {
         double d0 = x;
         double d1 = y;
         double d2 = z;
