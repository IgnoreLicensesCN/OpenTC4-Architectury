...
public class ObsidianWandRodItem extends ThaumcraftAspectRegenWandRodItem implements WorkAsWandRod, ICraftingCostAspectOwnerComponent<Aspect> {
    public ObsidianWandRodItem() {
        super(new Properties(), getAspectsCentiVisListWithValue(Aspects.EARTH,7 * CENTIVIS_MULTIPLIER));
    }

    private final CalcCacheableCentiVisList<Aspect> capacity = new CalcCacheableCentiVisList<>(
            getPrimalAspectCentiVisListWithValueCastedUnmodifiable(75 * CENTIVIS_MULTIPLIER),true
    );
    @Override
    @UnmodifiableView
