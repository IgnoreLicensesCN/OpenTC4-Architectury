...
//                 && (!this.level().isAirBlock(this.currentFlightTarget.posX, this.currentFlightTarget.posY, this.currentFlightTarget.posZ)
//                 || this.currentFlightTarget.posY < 1 || this.currentFlightTarget.posY > this.level().getHeightValue(
//                         this.currentFlightTarget.posX, this.currentFlightTarget.posZ
//         ) + 8 || this.level().getBiomeGenForCoords(this.currentFlightTarget.posX, this.currentFlightTarget.posZ).biomeID != Config.biomeTaintID)
//         ) {
...
//         this.motionX = this.motionX*(1-0.015) + (Math.signum(var1) * (double)0.5F) * 0.015;
//         this.motionY = this.motionY*(1-0.01)+(Math.signum(var3) * (double)0.7F) * (double)0.1F;
//         this.motionZ = this.motionZ*(1-0.015) + (Math.signum(var5) * (double)0.5F) * 0.015;
...
//         double var3 = this.entityToAttack.posY + (double)this.entityToAttack.getEyeHeight() - this.posY;//eyepos
//         double var5 = this.entityToAttack.posZ - this.posZ;
//         this.motionX += (Math.signum(var1) * (double)0.5F - this.motionX) * 0.025;
//         this.motionY += (Math.signum(var3) * (double)0.7F - this.motionY) * (double)0.1F;
//         this.motionZ += (Math.signum(var5) * (double)0.5F - this.motionZ) * 0.025;
//         float var7 = (float)(Math.atan2(this.motionZ, this.motionX) * (double)180.0F / Math.PI) - 90.0F;
//         float var8 = MathHelper.wrapAngleTo180_float(var7 - this.rotationYaw);
//         this.moveForward = 0.1F;
//         this.rotationYaw += var8;
...
//      if (this.attackTime <= 0 && par2 < 3.0F
