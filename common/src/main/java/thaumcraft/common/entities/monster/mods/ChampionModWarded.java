...
public class ChampionModWarded implements IChampionModifierEffect {
   ...
   public void showFX(EntityLivingBase boss) {
      if (!boss.level().rand.nextBoolean()) {
         ...
         Thaumcraft.proxy.drawGenericParticles(boss.level(), boss.boundingBox.minX + (double)w, boss.boundingBox.minY + (double)h, boss.boundingBox.minZ + (double)d, 0.0F, 0.0F, 0.0F, 0.5F + boss.level().rand.nextFloat() * 0.1F, 0.5F + boss.level().rand.nextFloat() * 0.1F, 0.5F + boss.level().rand.nextFloat() * 0.1F, 0.6F, true, 21, 4, 1, 4 + boss.level().rand.nextInt(4), 0, 0.8F + boss.level().rand.nextFloat() * 0.3F);
      }
   }
}
