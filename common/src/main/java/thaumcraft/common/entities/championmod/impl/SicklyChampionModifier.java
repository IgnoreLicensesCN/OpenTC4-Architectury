...
public class SicklyChampionModifier extends ChampionModifier implements IClientTickableChampionModifier, IAttackListenerChampionModifier {

    public SicklyChampionModifier(UUID uuid, ChampionModifierResourceLocation id) {
        super(uuid, id);
    }
    ...
    public void championModifierOnAttack(LivingEntity victim,LivingEntity attacker, DamageSource source, float amountNotReduced, float amount) {
        ...
        if (random.nextFloat() < 0.4F) {
            victim.addEffect(new MobEffectInstance(MobEffects.HUNGER,500));
        }
    }
