...
public class WarpChampionModifier extends ChampionModifier implements IClientTickableChampionModifier, IAttackListenerChampionModifier {

    public WarpChampionModifier(UUID uuid, ChampionModifierResourceLocation id) {
        super(uuid, id);
    }
    ...
    public void championModifierOnAttack(LivingEntity victim,LivingEntity attacker, DamageSource source, float amountNotReduced, float amount) {
        ...
        if (random.nextFloat() < 0.33F) {
            var warpInfo = WarpInfo.getFromLivingEntity(victim);
            if (warpInfo != null) {
                warpInfo.addTempWarpAndSync(victim,1 + random.nextInt(3));
            }
        }
    }
}
