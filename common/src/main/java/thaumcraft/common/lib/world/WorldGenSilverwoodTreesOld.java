...
public class WorldGenSilverwoodTreesOld extends WorldGenAbstractTree {
   ...
   public boolean generate(Level par1World, Random par2Random, int par3, int par4, int par5) {
      ...
      if (this.heightLimit == 0) {
         this.heightLimit = this.heightLimitLimit / 2 + this.rand.nextInt(this.heightLimitLimit);
      }

      int x = 0;
      int z = 0;

      for(int var11 = 0; var11 < this.trunkSize; ++var11) {
         for(int var12 = 0; var12 < this.trunkSize; ++var12) {
            if (!this.validTreeLocation(var11, var12)) {
               return false;
            }
         }
      }
