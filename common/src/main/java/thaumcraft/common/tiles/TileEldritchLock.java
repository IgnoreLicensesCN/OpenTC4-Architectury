...
public class TileEldritchLock extends TileThaumcraft {
   ...
   private void doBossSpawn() {
      this.level().playSoundEffect(this.xCoord, this.yCoord, this.zCoord, "thaumcraft:ice", 1.0F, 1.0F);
   ...
   private void spawnWardenBossRoom(int cx, int cz, int exit) {
      ...
      for(int a = -1; a <= 1; ++a) {
         for(int b = -1; b <= 1; ++b) {
            if (a != 0 && b != 0 && this.level().rand.nextFloat() < 0.9F) {
   ...
   private void spawnGolemBossRoom(int cx, int cz, int exit) {
      ...
      for(int a = -10; a <= 10; ++a) {
         for(int b = -10; b <= 10; ++b) {
            if ((a < -2 && b < -2 || a > 2 && b > 2 || a < -2 && b > 2 || a > 2 && b < -2) && this.level().rand.nextFloat() < 0.15F && this.level().isAirBlock(x + a, y + 2, z + b)) {
               float rr = this.level().rand.nextFloat();
               int md = rr < 0.05F ? 2 : (rr < 0.2F ? 1 : 0);
               this.level().setBlock(x + a, y + 2, z + b, this.level().rand.nextFloat() < 0.3F ? ConfigBlocks.blockLootCrate : ConfigBlocks.blockLootUrn, md, 3);
            }
         }
      }
   ...
   private void spawnCultistBossRoom(int cx, int cz, int exit) {
      ...
      for(int a = -4; a <= 4; ++a) {
         for(int b = -4; b <= 4; ++b) {
            if ((Math.abs(a) != 2 && Math.abs(b) != 2 || !this.level().rand.nextBoolean()) && (Math.abs(a) != 3 && Math.abs(b) != 3 || !(this.level().rand.nextFloat() > 0.33F)) && (Math.abs(a) != 4 && Math.abs(b) != 4 || !(this.level().rand.nextFloat() > 0.25F))) {
   ...
   private void spawnTaintBossRoom(int cx, int cz, int exit) {
      ...
      for(int a = -12; a <= 12; ++a) {
         for(int b = -12; b <= 12; ++b) {
            ...
            for(int c = 0; c < 9; ++c) {
               if (this.level().isAirBlock(x + b, y + 2 + c, z + a) && BlockUtils.isAdjacentToSolidBlock(this.level(), x + b, y + 2 + c, z + a) && this.level().rand.nextInt(3) != 0) {
                  this.level().setBlock(x + b, y + 2 + c, z + a, ConfigBlocks.blockTaintFibres, this.level().rand.nextInt(4) == 0 ? 1 : 0, 3);
               }
            }

            if ((double)this.level().rand.nextFloat() < 0.15) {
               this.level().setBlock(x + b, y + 2, z + a, ConfigBlocks.blockTaint, 0, 3);
               if ((double)this.level().rand.nextFloat() < 0.2) {
                  this.level().setBlock(x + b, y + 3, z + a, ConfigBlocks.blockTaint, 0, 3);
               }
            }

            if ((Math.abs(a) != 4 && Math.abs(b) != 4 || !this.level().rand.nextBoolean()) && (Math.abs(a) < 5 && Math.abs(b) < 5 || !(this.level().rand.nextFloat() > 0.33F)) && (Math.abs(a) < 7 && Math.abs(b) < 7 || !(this.level().rand.nextFloat() > 0.25F))) {
      ...
      EntityTaintacle boss2 = this.level().rand.nextBoolean() ? new EntityTaintacle(this.level()) : new EntityTaintacleGiant(this.level());
      ...
      EntityTaintacle boss4 = this.level().rand.nextBoolean() ? new EntityTaintacle(this.level()) : new EntityTaintacleGiant(this.level());
